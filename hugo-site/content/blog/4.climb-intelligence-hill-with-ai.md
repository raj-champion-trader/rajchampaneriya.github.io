...
tags: ["ai", "career", "software-engineering", "point-of-view"]
...
{{< tldr >}}
**In this post:** Four steps to use AI so you get *smarter*, not lazier — (1) **Intelligent Laziness** and the DRAG framework, (2) **Climbing the Intelligence Hill** with better prompting, (3) **The Intelligent Gym** so AI strengthens your thinking, (4) **The Intelligent Fool** and why "I don't know" is the real edge. Bottom line: **Use AI to think better, not less.**
{{< /tldr >}}
...
```mermaid
flowchart TB
  subgraph Curve1["Curve 1 — Capped payoff"]
    A[Formatting, emails, FYI meetings] --> B[Value flattens]
    B --> C["Satisficing: stop when good enough"]
  end
  subgraph Curve2["Curve 2 — Uncapped payoff"]
    D[Strategy, product, key relationships] --> E[Value flat for a while]
    E --> F[Then goes to the moon]
  end
  Curve1 ~~~ Curve2
```
...
```mermaid
flowchart TB
  Z[Zero-shot: roll the dice] --> C1[Camp 1: One-shot]
  C1 --> C2[Camp 2: Few-shot]
  C2 --> C3[Camp 3: Chain of thought]
  C3 --> C4[Camp 4: Agents]
  C4 --> E[Elite results]
  style C1 fill:#e8f4fc
  style C2 fill:#e8f4fc
  style C3 fill:#e8f4fc
  style C4 fill:#e8f4fc
```
