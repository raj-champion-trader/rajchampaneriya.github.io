...
```mermaid
graph LR
    A["<b>Autocomplete</b><br/>Inline code suggestions"] --> B["<b>Chat</b><br/>Conversational coding"]
    B --> C["<b>CLI Agent</b><br/>Task-level delegation"]
    C --> D["<b>SDK</b><br/>Programmatic integration"]
...
```mermaid
flowchart TD
    A[Raw Transcript] --> B[Copilot SDK Client]
    B --> C{Copilot CLI - Server Mode}
    ...
    style H fill:#e8f4fd,stroke:#1a73e8,color:#000
    style I fill:#e8f4fd,stroke:#1a73e8,color:#000
```
...
graph TD
    subgraph Workflow["Composable AI Workflows"]
        A[Code Review Agent] --> D[Orchestrator App]
    ...
    style F fill:#e8f4fd,stroke:#1a73e8,color:#000
```
