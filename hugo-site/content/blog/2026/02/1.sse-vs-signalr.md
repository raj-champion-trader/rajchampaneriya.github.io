...
```mermaid
graph LR
    RS[Redis Stream] --> RSC[RedisStreamConsumer] --> TB[TickBroadcaster]
    TB --> PCC[Per-Client Channels] --> SSE[SSE Response]
    TB --> ATC[Aggregator Channel] --> CA[CandlestickAggregator] --> PCCC[Per-Client Candle Channels] --> SSE

    classDef pipeline fill:#fff3e0,stroke:#ef6c00,stroke-width:1px;
    class RS,RSC,TB,PCC,ATC,CA,PCCC,SSE pipeline;
```
