...
graph TD
    subgraph Publisher
    MS["MarketSimulator<br/>(BackgroundService)"] -->|Generates ticks| MIA["MarketIngestion.Api<br/>(Minimal API)"]
    end

    MIA -->|"Validates & publishes"| RS["Redis Streams<br/>(Consumer Group: 'brokers')"]

    subgraph Consumer
    RS -->|"Reads batches"| RSC["RedisStreamConsumer<br/>(BackgroundService)"]
    RSC -->|"Deserialize & Ack"| TB["TickBroadcaster"]
    end

    TB -->|BroadcastTick| SCM["SseClientManager"]
    SCM -->|"Per-client tick channels"| SSE1(("SSE Stream"))

    TB -->|AggregatorTickChannel| CA["CandlestickAggregator"]
    CA -->|"Per-client candle channels"| SSE2(("SSE Stream"))
    ...
    class MS,MIA,RSC,TB,SCM,CA service;
    class RS storage;
    class SSE1,SSE2 stream;
```
